                    ...
                    <div class="header-title">
                        ...
                        @if ($filters['year'] || $filters['month'])
                            <div style="font-size: 11px; margin-top: 5px;">
                                @if ($filters['month'])
                                    Bulan
                                    {{ \Carbon\Carbon::create(2000, $filters['month'], 1)->translatedFormat('F') }}
                                @endif
                                @if ($filters['year'])
                                    Tahun {{ $filters['year'] }}
                                @endif
                        ...
                        <tbody>
                            ...
                            @forelse($transactions as $index => $transaction)
                                ...
                                <tr>
                                    ...
                                    <td class="center">
                                        {{ \Carbon\Carbon::parse($transaction->first()->tanggal_masuk)->format('d F Y') }}
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        @endif
                                        @if (
                                            !$transaction->first()->nomor_dokumen &&
                                                !$transaction->first()->nomor_faktur &&
                                                !$transaction->first()->nomor_surat_jalan)
                                            -
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        <ul>
                                            ...
                                            @foreach ($groupedItems as $item)
                                                <li>
                                                    {{ $item->barang_nama }} ({{ number_format($item->jumlah) }}
                                                    {{ $item->satuan }})
                                                </li>
                                            @endforeach
                                        </ul>
                            ...
                            <tr class="total-row">
                                <td colspan="5" class="right" style="padding-right: 15px;">TOTAL ITEM MASUK
                                    KESELURUHAN</td>
                                <td class="bold center">
                                    {{ number_format($total_all_items) }} unit dari {{ $transactions->count() }}
                                    transaksi
                                </td>
                            </tr>
                        </tbody>
